...
namespace LucAdm.Tests
{
    public class CreateUserTests : IClassFixture<ServiceFixture>
    {
        ...
        public void CreateUser_With_Correct_Command_Should_Create_User()
        {
            ...
            var response = Some.UserService(context, userRepository).CreateUser(command);
        ...
        [NamedFact, Trait("Category", "Unit")]
        public void CreateUser_Invalid_Email_Should_Return_Validation_Error()
        {
            var response = Some.UserService().CreateUser(Some.CreateUserCommand().With(email: "invalid"));
            response.ValidationResult.Errors.Should().ContainKey(PropertyName.Get((CreateUserCommand x) => x.Email));
        }

        [NamedFact, Trait("Category", "Unit")]
        public void CreateUser_Invalid_UserName_Should_Return_Validation_Error()
        {
            var response = Some.UserService().CreateUser(Some.CreateUserCommand().With(userName: "!%^^#$#$"));
            response.ValidationResult.Errors.Should().ContainKey(PropertyName.Get((CreateUserCommand x) => x.UserName));
        }
        ...
        [NamedFact, Trait("Category", "Unit")]
        public void CreateUser_With_Too_Short_Password_Should_Return_Validation_Error()
        {
            var response = Some.UserService().CreateUser(Some.CreateUserCommand().With(password: "sh", rePassword: "sh"));
            response.ValidationResult.Errors.Should().ContainKey(PropertyName.Get((CreateUserCommand x) => x.Password));
        }
        ...
        {
            var response = Some.UserService(Some.MockedContext().With(Some.User().With(userName: "existingName").ToList()))
                .CreateUser(Some.CreateUserCommand().With(userName: "existingName"));

            response.ValidationResult.Errors.Should().ContainKey(PropertyName.Get((CreateUserCommand x) => x.UserName));
        }

        [NamedFact, Trait("Category", "Unit")]
        public void CreateUser_With_Duplicated_Email_Should_Return_Validation_Error()
        {
            var response = Some.UserService(Some.MockedContext().With(Some.User().With(userName: "u1", email: "existing@email.com").ToList()))
                .CreateUser(Some.CreateUserCommand().With(userName: "u2", email: "existing@email.com"));
